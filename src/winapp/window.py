...
__hdc = user32.GetDC(0)
DPI_Y = gdi32.GetDeviceCaps(__hdc, LOGPIXELSY)
user32.ReleaseDC(0, __hdc)
...
class Window(object):
    ...
    def __init__(
        self, window_class='', style=WS_CHILD | WS_VISIBLE, ex_style=0,
        left=0, top=0, width=0, height=0, window_title=None, hmenu=0,
        parent_window=None, wrap_hwnd=None, hinstance=0
    ):
    ...
    def enable_window(self, flag):
        user32.EnableWindow(self.hwnd, int(flag))
...
#        self.hfont = hfont
    ...
    def set_font(self, font_name=None, font_size=8, font_weight=FW_DONTCARE, font_italic=FALSE, font_underline=FALSE, hfont=None):
        if self.hfont:
            gdi32.DeleteObject(self.hfont)
        if font_name:
            cHeight = -kernel32.MulDiv(font_size, DPI_Y, 72)
            hfont = gdi32.CreateFontW(cHeight, 0, 0, 0, font_weight, font_italic, font_underline, FALSE, ANSI_CHARSET, OUT_TT_PRECIS,
                    CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, font_name)
        elif hfont is None:
